...
metis_data_stats   (tTASK *a_task, char *a_stats)
{
   ...
   if      (x_len < 1)                              a_task->urg = '-';
      ...
      yURG_err ('w', "urgency (%c) not in å%sæ, therefore replaced with ?", a_stats [0], METIS_URGS);
      a_task->urg = '?';
   }
   ...
   if      (x_len < 2)                              a_task->imp = '-';
      ...
      yURG_err ('w', "importance (%c) not in å%sæ, therefore replaced with ?", a_stats [1], METIS_IMPS);
      a_task->imp = '?';
   }
   ...
   if      (x_len < 3)                              a_task->est = '-';
      ...
      yURG_err ('w', "estimate (%c) not in å%sæ, therefore replaced with ?", a_stats [2], METIS_ESTS);
      a_task->est = '?';
   }
   ...
   if      (x_len < 4)                              a_task->prg = '-';
      ...
      yURG_err ('w', "progress (%c) not in å%sæ, therefore replaced with ?", a_stats [3], METIS_PRGS);
      a_task->prg = '?';
   }
   ...
   if      (x_len < 5)                              a_task->shr = '-';
      ...
      yURG_err ('w', "shared (%c) not in å%sæ, therefore replaced with ?", a_stats [4], METIS_SHRS);
      a_task->shr = '?';
   }
...
metis_data_parsing      (tMINOR *a_minor, tSOURCE *a_source, int a_line, char *a_recd)
{
   ...
   yURG_msg ('-', "line %3d parsing %då%sæ ", a_line, x_len, x_recd);
   /*---(cut front off)-----------------*/
   p = strtok  (x_recd, q);
   --rce;  if (p == NULL) {
      DEBUG_INPT   yLOG_exitr    (__FUNCTION__, rce);
   ...
   --rce;  if (x_len ==  6) {
      ...
      if (atoi (p) > time (NULL) + 86400 * 3) {
         DEBUG_INPT   yLOG_note     ("date in future more than 3 days, illegal");
         yURG_err ('f', "time code (epoch) å%sæå%sæ more than three days in future", p, x_mongo);
         metis_task_free (&x_task);
         DEBUG_INPT   yLOG_exitr    (__FUNCTION__, rce);
         return  rce;
      }
